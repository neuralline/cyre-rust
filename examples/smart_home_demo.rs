...
async fn setup_smart_home_devices(cyre: &mut Cyre) {
    ...
    // Smart Light Control
    cyre.action(IO::new("light.control")
        .with_name("Smart Light Controller")
        .with_priority(Priority::High));
    
    cyre.on("light.control", |payload| {
        Box::pin(async move {
            let room = payload.get("room").and_then(|v| v.as_str()).unwrap_or("living_room");
            let action = payload.get("action").and_then(|v| v.as_str()).unwrap_or("toggle");
    ...
    // Scene Control
    cyre.action(IO::new("scene.activate")
        .with_name("Scene Controller")
        .with_priority(Priority::Medium));
    
    cyre.on("scene.activate", |payload| {
        Box::pin(async move {
            let scene = payload.get("scene").and_then(|v| v.as_str()).unwrap_or("relaxing");
    ...
    // Thermostat Control
    cyre.action(IO::new("climate.control")
        .with_name("Smart Thermostat")
        .with_throttle(2000)); // Prevent rapid changes
    ...
    cyre.action(IO::new("security.motion")
        .with_name("Motion Detector")
        .with_change_detection()); // Only trigger on actual changes
    
    cyre.on("security.motion", |payload| {
        Box::pin(async move {
            let location = payload.get("location").and_then(|v| v.as_str()).unwrap_or("unknown");
            let detected = payload.get("detected").and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    // Door Lock Control
    cyre.action(IO::new("security.lock")
        .with_name("Smart Lock")
        .with_priority(Priority::Critical)); // Security is critical
    
    cyre.on("security.lock", |payload| {
        Box::pin(async move {
            let action = payload.get("action").and_then(|v| v.as_str()).unwrap_or("lock");
            let door = payload.get("door").and_then(|v| v.as_str()).unwrap_or("front_door");
            let user = payload.get("user").and_then(|v| v.as_str()).unwrap_or("system");
    ...
    cyre.action(IO::new("energy.monitor")
        ...
        .with_debounce(1000)); // Smooth out rapid readings
    
    cyre.on("energy.monitor", |payload| {
        Box::pin(async move {
            let device = payload.get("device").and_then(|v| v.as_str()).unwrap_or("house");
            let consumption = payload.get("consumption").and_then(|v| v.as_f64()).unwrap_or(0.0);
    ...
    // Smart TV Control
    cyre.action(IO::new("entertainment.tv")
        .with_name("Smart TV")
        .with_priority(Priority::Low)); // Entertainment is lower priority
    ...
    cyre.on("entertainment.tv", |payload| {
        Box::pin(async move {
            ...
            let volume = payload.get("volume").and_then(|v| v.as_u64()).unwrap_or(50);
    ...
    cyre.action(IO::new("automation.rule")
        .with_name("Automation Engine")
        .with_priority(Priority::High));
    
    cyre.on("automation.rule", |payload| {
        Box::pin(async move {
            let rule = payload.get("rule").and_then(|v| v.as_str()).unwrap_or("unknown");
            let trigger = payload.get("trigger").and_then(|v| v.as_str()).unwrap_or("manual");
...
async fn morning_routine_demo(cyre: &Cyre) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 2. Adjust climate
    let climate_result = cyre.call("climate.control", json!({
        "temperature": 21.0,
        "mode": "heat"
    })).await;
    ...
    let light_result = cyre.call("light.control", json!({
        "room": "kitchen",
        "action": "on",
        "brightness": 90
    })).await;
    ...
    let automation_result = cyre.call("automation.rule", json!({
        "rule": "welcome_home",
...
async fn security_system_demo(cyre: &Cyre) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. Motion detection
    let motion_result = cyre.call("security.motion", json!({
        "location": "front_door",
        "detected": true
    })).await;
    ...
    let lock_result = cyre.call("security.lock", json!({
        "door": "front_door",
        "action": "unlock",
        "user": "john_doe"
    })).await;
    ...
    let motion_result2 = cyre.call("security.motion", json!({
    ...
    // 4. Lock door again
    let lock_result2 = cyre.call("security.lock", json!({
        "door": "front_door",
        "action": "lock",
        "user": "automatic"
    })).await;
...
async fn energy_management_demo(cyre: &Cyre) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (device, consumption) in devices {
        let energy_result = cyre.call("energy.monitor", json!({
            "device": device,
        ...
        if energy_result.ok {
            ...
            println!("   📈 {}: {} efficiency (${:.2}/hr)", device, efficiency, cost.as_f64().unwrap());
        }
...
async fn entertainment_system_demo(cyre: &Cyre) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Control TV
    let tv_result = cyre.call("entertainment.tv", json!({
        "action": "on",
        "channel": "Netflix",
        "volume": 75
    })).await;
...
async fn real_time_monitoring_demo(cyre: &Cyre) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=5 {
        ...
        let temp_result = cyre.call("climate.control", json!({
