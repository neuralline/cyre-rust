...
async fn test_fast_path_performance() -> u64 {
    ...
    let fast_config = IO::new("fast-test");
    println!("🔍 Is fast path: {}", is_fast_path(&fast_config));
    println!("📊 Operator count: {}", estimate_operator_count(&fast_config));

    // Register fast path action
    cyre.action(fast_config).unwrap();
    cyre.on("fast-test", |payload| {
...
async fn test_pipeline_performance() -> u64 {
    ...
    println!("🔍 Is fast path: {}", is_fast_path(&pipeline_config));
    println!("📊 Operator count: {}", estimate_operator_count(&pipeline_config));

    cyre.action(pipeline_config).unwrap();
    cyre.on("pipeline-test", |payload| {
...
async fn test_compiler_vs_executor() {
    ...
    for (name, config) in &configs {
        println!(
            "   {} - Fast path: {}, Operators: {}, Performance: {} ops/sec",
            name,
            is_fast_path(config),
            estimate_operator_count(config),
            format_number(estimated_performance(config) as u64)
        );
    }
...
async fn test_system_integration() {
    ...
    cyre.on("integration-test", |payload| {
        Box::pin(async move {
