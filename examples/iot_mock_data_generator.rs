...
impl SmartHomeSystem {
    ...
    async fn start_device_simulators(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for location in &locations {
                    ...
                    let temperature = base_temp + rng.gen_range(-3.0..3.0);
        ...
        tokio::spawn(async move {
            ...
            loop {
                let delay = rng.gen_range(10..60); // 10-60 seconds
                sleep(Duration::from_secs(delay)).await;
                
                let location = &locations[rng.gen_range(0..locations.len())];
                let motion_detected = rng.gen_bool(0.3); // 30% chance of motion
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (device, range) in &devices {
                    let consumption = rng.gen_range(range.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for room in &rooms {
                    let brightness = rng.gen_range(20..100);
                    let state = if rng.gen_bool(0.8) { "on" } else { "off" };
    ...
    async fn start_automation_engine(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let (rule_name, actions) = &automation_rules[rng.gen_range(0..automation_rules.len())];
...
impl SmartHomeSystem {
    ...
    fn generate_sensor_reading(sensor_type: &str, location: &str) -> SensorReading {
        ...
        let (value, unit) = match sensor_type {
            "temperature" => (rng.gen_range(18.0..26.0), "°C".to_string()),
            "humidity" => (rng.gen_range(30.0..70.0), "%".to_string()),
            "air_quality" => (rng.gen_range(50.0..150.0), "AQI".to_string()),
            "noise_level" => (rng.gen_range(25.0..65.0), "dB".to_string()),
            "light_level" => (rng.gen_range(0.0..1000.0), "lux".to_string()),
    ...
    async fn simulate_device_issues(&self) {
        ...
        if rng.gen_bool(0.05) { // 5% chance
            ...
            if rng.gen_bool(0.8) { // 80% recovery success rate
