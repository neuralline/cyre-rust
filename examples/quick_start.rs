// examples/rust_comeback_test.rs
...
/// Concurrent channels test - true parallelism
async fn concurrent_channels_test() -> BenchmarkResult {
    println!("\n🔀 CONCURRENT CHANNELS TEST - Unleash the parallelism!");
    
    let cyre = std::sync::Arc::new(tokio::sync::Mutex::new(Cyre::new()));
    let num_channels = 20;
    let ops_per_channel = 10_000;
    ...
    // Setup multiple channels
    {
        ...
        for i in 0..num_channels {
            let channel_id = format!("parallel-{}", i);
            cyre_guard.action(IO::new(&channel_id));
            cyre_guard.on(&channel_id, |payload| {
                Box::pin(async move {
                    CyreResponse {
                        ok: true,
                        payload,
    ...
    let payload = json!({"parallel": true});
    
    println!("⚡ Running {} ops across {} channels concurrently...", total_operations, num_channels);
    let start = Instant::now();
    
    // Create all tasks - but use sequential execution to avoid Send issues
    let mut tasks = Vec::new();
    for i in 0..num_channels {
        let channel_id = format!("parallel-{}", i);
        let payload_clone = payload.clone();
        let cyre_clone = std::sync::Arc::clone(&cyre);
        
        let task = async move {
            for _ in 0..ops_per_channel {
                let cyre_guard = cyre_clone.lock().await;
                let _result = cyre_guard.call(&channel_id, payload_clone.clone()).await;
            }
        };
    ...
    // Execute all tasks sequentially for now (to avoid Send issues)
    for task in tasks {
        task.await;
    }
    ...
    let duration_ms = duration.as_secs_f64() * 1000.0;
    let ops_per_sec = (total_operations as f64 / duration.as_secs_f64()) as u64;
    let avg_latency_us = 0.0; // Sequential execution
    
    println!("🎯 Result: {} ops/sec", ops_per_sec);
    
    BenchmarkResult {
        name: "Concurrent Channels".to_string(),
        ops_per_sec,
        total_ops: total_operations,
        duration_ms,
        avg_latency_us,
...
async fn zero_allocation_test() -> BenchmarkResult {
    ...
    cyre.on("zero-alloc", |payload| {
        Box::pin(async move {
            // Return the exact same payload - zero allocations!
            CyreResponse {
                ok: true,
...
fn display_results(results: &[BenchmarkResult]) {
    ...
    let peak_performance = results.iter().map(|r| r.ops_per_sec).max().unwrap_or(0);
