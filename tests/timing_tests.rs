...
async fn test_infinite_repeat() {
    ...
    cyre.action(
        IO::new("infinite-test")
            .with_interval(200)
            .with_repeat_infinite()
    ).unwrap();
    
    cyre.on("infinite-test", move |_| {
        let counter = counter_clone.clone();
        Box::pin(async move {
            counter.fetch_add(1, Ordering::SeqCst);
...
async fn test_very_short_intervals() {
    ...
    let result = cyre.action(
        IO::new("short-interval-test")
            .with_interval(1)
            .with_repeat_count(10)
    );

    if result.is_ok() {
        cyre.on("short-interval-test", move |_| {
            let counter = counter_clone.clone();
            Box::pin(async move {
                counter.fetch_add(1, Ordering::SeqCst);
                CyreResponse::success(json!({}), "OK")
...
async fn test_very_long_delays() {
    ...
    cyre.on("long-delay-test", move |_| {
        let counter = counter_clone.clone();
        Box::pin(async move {
            counter.fetch_add(1, Ordering::SeqCst);
            CyreResponse::success(json!({}), "OK")
...
async fn test_concurrent_timed_actions() {
    ...
    cyre.call("interval-test", json!({})).await;
    sleep(Duration::from_millis(600)).await;

    // Should execute once (no repeat specified)
    assert_eq!(counter.load(Ordering::SeqCst), 1);
}
...
async fn test_interval_plus_repeat() {
    ...
    cyre.action(
        IO::new("interval-repeat-test")
            .with_interval(300)
            .with_repeat_count(4)
    ).unwrap();
    
    cyre.on("interval-repeat-test", move |_| {
        let counter = counter_clone.clone();
        Box::pin(async move {
            counter.fetch_add(1, Ordering::SeqCst);
            CyreResponse::success(json!({}), "OK")
...
async fn test_delay_plus_repeat() {
    ...
    cyre.action(
        IO::new("delay-repeat-test")
            .with_delay(500)
            .with_repeat_count(3)
    ).unwrap();
    
    cyre.on("delay-repeat-test", move |_| {
        let counter = counter_clone.clone();
        Box::pin(async move {
            counter.fetch_add(1, Ordering::SeqCst);
            CyreResponse::success(json!({}), "OK")
...
#[tokio::test]
async fn test_full_combo_delay_interval_repeat() {
    let mut cyre = Cyre::new();
    cyre.init().await.unwrap();

    let counter = Arc::new(AtomicU32::new(0));
    let times = Arc::new(std::sync::Mutex::new(Vec::<u64>::new()));
    let counter_clone = counter.clone();
    let times_clone = times.clone();
    let start_time = std::time::Instant::now();

    cyre.action(
        IO::new("full-combo-test")
            .with_delay(400)        // Start after 400ms
            .with_interval(300)     // Then every 300ms
            .with_repeat_count(3)   // 3 times total
    ).unwrap();
    
    cyre.on("full-combo-test", move |_| {
        let counter = counter_clone.clone();
