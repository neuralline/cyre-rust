...
pub fn require_fields<'a>(
    fields: &'a [&'a str]
) -> impl (Fn(ActionPayload) -> ValidationResult) + 'a {
    move |payload| {
...
pub fn validate_numeric_range(
    ...
    min: f64,
    max: f64
) -> impl (Fn(ActionPayload) -> ValidationResult) + '_ {
...
pub fn field_equals(field: &str, expected: Value) -> impl (Fn(ActionPayload) -> bool) + '_ {
    let field = field.to_string();
    move |payload| { payload.get(&field) == Some(&expected) }
}

/// Check if field exists
pub fn field_exists(field: &str) -> impl (Fn(ActionPayload) -> bool) + '_ {
    let field = field.to_string();
    move |payload| { payload.get(&field).is_some() }
}
...
pub fn field_contains<'a>(
    field: &'a str,
    pattern: &'a str
) -> impl (Fn(ActionPayload) -> bool) + 'a {
    let field = field.to_string();
...
pub fn rename_field<'a>(
    old_name: &'a str,
    new_name: &'a str
) -> impl (Fn(ActionPayload) -> ActionPayload) + 'a {
...
pub fn increment_field(
    field_name: &str,
    amount: f64
) -> impl (Fn(ActionPayload) -> ActionPayload) + '_ {
    let field_name = field_name.to_string();
    move |mut payload| {
        if let Some(obj) = payload.as_object_mut() {
...
pub fn composite_validation(
    validators: Vec<Box<dyn Fn(ActionPayload) -> ValidationResult>>
) -> impl Fn(ActionPayload) -> ValidationResult {
    move |payload| {
...
pub fn all_conditions(
    conditions: Vec<Box<dyn Fn(ActionPayload) -> bool>>
) -> impl Fn(ActionPayload) -> bool {
    move |payload| { conditions.iter().all(|condition| condition(payload.clone())) }
}

/// Create a composite condition (OR logic)
pub fn any_condition(
    conditions: Vec<Box<dyn Fn(ActionPayload) -> bool>>
) -> impl Fn(ActionPayload) -> bool {
    move |payload| { conditions.iter().any(|condition| condition(payload.clone())) }
}
