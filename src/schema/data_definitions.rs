...
pub fn validate_id(value: &JsonValue) -> DataDefResult {
    if !is_string(value) || value.as_str().unwrap_or("").trim().is_empty() {
        return DataDefResult::error(
            format!(
                "Channel ID must be a non-empty text value, but received {}",
...
pub fn validate_detect_changes(value: &JsonValue) -> DataDefResult {
    ...
    DataDefResult::success(value.clone(), Some("detectChanges".to_string()))
}
...
pub fn validate_repeat(value: &JsonValue) -> DataDefResult {
    ...
    if !is_number(value) && !is_boolean(value) {
        return DataDefResult::error(
            format!(
                "Repeat must be a number, true, or false, but received {}",
                describe_value(value)
            ),
            false,
            vec![
...
mod tests {
    ...
    fn test_id_validation() {
        // Valid ID
        let result = validate_id(&json!("test-action"));
        assert!(result.ok);
        assert_eq!(result.talent_name, Some("schema".to_string()));

        // Invalid ID
        let result = validate_id(&json!(""));
        assert!(!result.ok);
        assert_eq!(result.blocking, Some(true));
    }

    #[test]
    fn test_throttle_validation() {
        // Valid throttle
        let result = validate_throttle(&json!(1000));
        assert!(result.ok);
        assert_eq!(result.talent_name, Some("throttle".to_string()));

        // Invalid throttle
        let result = validate_throttle(&json!("not-a-number"));
        assert!(!result.ok);
        assert!(result.suggestions.as_ref().unwrap().len() > 0);
    }

    #[test]
    fn test_required_validation() {
        // Valid required (boolean)
        let result = validate_required(&json!(true));
        assert!(result.ok);
        assert_eq!(result.talent_name, Some("required".to_string()));

        // Valid required (non-empty)
        let result = validate_required(&json!("non-empty"));
        assert!(result.ok);
        assert_eq!(result.talent_name, Some("required".to_string()));

        // Invalid required
        let result = validate_required(&json!("invalid"));
        assert!(!result.ok);
    }

    #[test]
    fn test_field_dispatcher() {
        // Test O(1) dispatcher
        let result = validate_field("throttle", &json!(1000));
        assert!(result.ok);
        assert_eq!(result.talent_name, Some("throttle".to_string()));

        // Test unknown field
        let result = validate_field("unknown", &json!("value"));
        assert!(!result.ok);
    }

    #[test]
    fn test_talent_categories() {
        assert!(PROTECTION_TALENTS.contains(&"throttle"));
        assert!(PROCESSING_TALENTS.contains(&"required"));
        assert!(SCHEDULING_TALENTS.contains(&"delay"));
    }
}
